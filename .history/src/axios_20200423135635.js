...
class Axios {
  ...
  get(...args) {
    if (args.length === 1 && typeof args[0] === "string") {
      ...
      args.length === 1 &&
      Object.prototype.toString.call(args[0]) === "Object"
    ) {
      // 如果之传入了一个对象参数
      options = { ...args[0], method: "get" };
    } else {
      // 进行别的操作
    }
  }
  post(...args) {
    if (args.length === 1 && typeof args[0] === "string") {
      options = { method: "post", url: args[0] };
    } else if (
      args.length === 1 &&
      Object.prototype.toString.call(args[0]) === "Object"
    ) {
      options = { ...args[0], method: "post" };
    } else {
    }
  }
  delete(...args) {
    if (args.length === 1 && typeof args[0] === "string") {
      options = { method: "delete", url: args[0] };
    } else if (
      args.length === 1 &&
      Object.prototype.toString.call(args[0]) === "Object"
    ) {
      options = { ...args[0], method: "delete" };
    } else {
    }
  }
}
